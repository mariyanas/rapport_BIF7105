...
```{r fig1, fig.cap = "plotMDS(y)", fig.width = 3, fig.height = 2.5, fig.align='center', size="small"}
par(mar = c(4, 4, 0, 1))
...
tumoraux et les tissus normaux chez les patients. Ici, nous avons un modèle linéaire additif. Le facteur de blocage est le `Patient`, comme ça nous pouvons isoler les effets dus au facteur `Tissue`. Les différences sont dues aux tissus, et non aux patients. Pour se faire, il faut d'abord créer une matrice de design selon le design expérimental.     
...
tests de rapport de vraisemblance pour identifier les gènes significatifs. Il fait un test pour l'effet *tissus tumoraux vs normaux*.   
...
```{r, fig3, fig.cap = "plotMD(lrt)", fig.width = 4.5, fig.height = 3.5, fig.align='center', size="small"}
par(mar = c(4, 4, 1.5, 1))
...
processus biologiques (BP). Les gènes régulés à la hausse dans les tumeurs 
...
Les gènes régulés à la baisse dans les tumeurs 
...
Pour conclure, nous avons démontré que l'outil **edgeR** est important pour l'analyse de données omiques issus du séquençage à haut débit, surtout ceux issus du séquençage de l'ARN. L’expression différentielle entre les tissus carcinomes épidermoïdes buccaux et les tissus normaux a été établie, ainsi qu'une analyse d'ontologie des gènes. Bien que notre analyse soit basé sur uniquement un facteur, l'approche de modèle linéaire généralisé de l'outil permet des analyses beaucoup plus complexes, comme les expérimentations à facteurs multiples et les analyses de séries temporelles par exemple. De plus, **edgeR** a été le premier à utiliser la loi binomiale négative pour représenter les données de comptages, ce qui lui permet de faire des bonnes analyses, autant pour les petits jeux de données comme le nôtre que les gros [@edgeRv4]. Facile et simple d'approche, cet outil nous a permit de mieux comprendre les différences entre deux tissus et d'en tirer des conclusions significatives. **edgeR** est et continuera à être un outil indispensable dans la recherche génomique et de biologie moléculaire.
